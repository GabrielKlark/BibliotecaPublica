(() => {
    ...
    btnSearch.addEventListener("click", async function (event) {
        ...
        const result = await fetch('/BibliotecaPublica/php/searchEst.php', {
                        method: 'POST',
                        body: dados
                    }).then((res) => res.json())
        ...
        if(resultLength == 0)
        {
            //alert(`Nenhum livro de isbn "${inpSearch.value}" em estoque encontrado para ser listado...`);
            tata.error('Nada!', `Nenhum livro de isbn "${inpSearch.value}" em estoque encontrado para ser listado...`, {
                position: 'tr',
                animate: 'slide',
                duration: 5000
            })
    ...
    btnSearchAll.addEventListener("click", async function(event) {  
        ...
        let resp = confirm("Tem certeza que deseja listar todos livros em estoque? (Isso pode gerar lentidão e falhas de sistema)");
        ...
        if(resp)
        {
            ...
            if(resultLength == 0)
            {
                //alert("Nenhum livro em estoque encontrado para ser listado...");
                tata.error('Nada!', `Nenhum livro em estoque encontrado para ser listado...`, {
                    position: 'tr',
                    animate: 'slide',
                    duration: 5000
                })
